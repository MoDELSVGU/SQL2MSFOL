...
//			NamingConvention.saveSelIndex(selName, ps);
	...
	public static void defineFunction(Select select, SelectPattern pattern) throws Exception {
		...
		case SELECT_FROM_WHERE: {
			String definition = "(assert (forall ((x Int)) (= (index-%1$s x) (and (index-%2$s x) (= (index-%3$s x) TRUE)))))";
			System.out.println(String.format(definition, NamingConvention.generateSelName(),
					NamingConvention.getSelName(ps.getFromItem()),
		...
		case SELECT_FROM_JOIN: {
			...
			String definition2 = "(assert (forall ((x Int) (y Int)) (=> (and (index-%1$s x) (index-%1$s y) (not (= x y))) (not (and (= (left x) (left y)) (= (right x) (right y)))))))";
			System.out.println(String.format(definition2, NamingConvention.generateSelIntermediateName()));
			String definition3 = "(assert (forall ((x Int)) (=> (index-%1$s x) (exists ((y Int) (z Int)) (and (index-%2$s y) (index-%3$s z) (= y (left x) (= z (right x))))))))";
			...
			String definition4 = "(assert (forall ((y Int) (z Int)) (=> (and (index-%1$s y) (index-%2$s z)) (exists ((x Int)) (and (index-$3$s x) (= y (left x)) (= z (right x)))))))";
		...
		case SELECT_FROM_JOIN_ON_WHERE: {
			String definition = "(assert (forall ((x Int)) (= (index-%1$s x) (and (index-%2$s x) (= (%3$s x) TRUE) (= (%4$s x) TRUE)))))";
